import type { Hobby, MbtiType, Rank } from './types';

/** 相性計算に必要な最小プロフィール(アプリのProfile/Person、サーバーのDB行の両方が満たす) */
export interface CompatProfile {
  mbti: MbtiType;
  hobbies: Hobby[];
}

// アプリ仕様書 4.4: MBTI 4軸から決定的にスコアを算出するモック用ヒューリスティック
...
export function totalScore(me: CompatProfile, p: CompatProfile): number {
...
export function compat(me: CompatProfile, p: CompatProfile): Compat {
